...
GLTextureBlitter::~GLTextureBlitter() { deinit(); }
...
void GLTextureBlitter::init(int width, int height)
{
    ...
    CHECK_GL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_width, m_height, 0,
...
{
    CHECK_GL(glBindTexture(GL_TEXTURE_2D, m_texture));
    CHECK_GL(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_width, m_height, GL_RGB,
...
void GLTextureBlitter::blit()
{
    ...
    CHECK_GL(glBindTexture(GL_TEXTURE_2D, m_texture));
    CHECK_GL(glFramebufferTexture(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                  m_texture, 0));
    CHECK_GL(glBlitFramebuffer(0, 0, m_width, m_height, 0, 0, m_width, m_height,
